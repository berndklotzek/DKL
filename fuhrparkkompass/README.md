...
assets/js/truck.js          3D-Lkw im Hero (Three.js, prozedural)
...
`truck.js` baut einen europäischen Sattelzug aus Grundkörpern (Fahrerhaus mit
Dachspoiler, Tanks, Spiegel, Auflieger mit Seitenschürzen, 22 Räder mit
Radschrauben, Fahrlicht, Umrissleuchten). Der Schriftzug auf dem Auflieger
wird zur Laufzeit aus einem Canvas gezeichnet. Der Zug fährt beim Laden ins
Bild, die Räder drehen, die Fahrbahnmarkierung läuft; die Maus neigt die Kamera,
Scrollen dreht den Zug.
...
- Farben stehen oben in `truck.js` unter `C`, Kameraposition in `camBase`.
